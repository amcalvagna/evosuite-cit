 ...
 */
public class CitObserver extends ExecutionObserver {

    //private Map<Integer, Set<InputCoverageGoal>> inputCoverage = new LinkedHashMap<>();
    private Map<Integer, Set<CitCoverageGoal>> citCoverage = new LinkedHashMap<>();

    private static final Logger logger = LoggerFactory.getLogger(CitObserver.class);
    ...
    public void afterStatement(Statement statement, Scope scope, Throwable exception) {
        if (statement instanceof EntityWithParametersStatement) {
            ...
            citCoverage.put(statement.getPosition(), CitCoverageFactory.createCoveredGoalsFromParameters(className, methodName, methodDesc, argObjects));
    ...
    public void testExecutionFinished(ExecutionResult r, Scope s) {
        int total=0; 
        for ( int entry : citCoverage.keySet()) total+=citCoverage.get(entry).size();
        
        logger.trace("Attaching {} map entries with {} total cit goals to the test ExecutionResult", citCoverage.size(), total);
        //r.setInputGoals(inputCoverage);
        r.setCitGoals(citCoverage);
    }
    ...
    @Override
    public void clear() {
        logger.trace("Clearing Observer data");
        //inputCoverage = new LinkedHashMap<>();
        citCoverage = new LinkedHashMap<>();
    }
